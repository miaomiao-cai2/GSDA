...
import dataloader, utils, mini_batch_test
import ast
import metrics
...
def main_args():
    ...
    ##OOD
    args.add_argument('--dataset_path', default='./dataset', type=str)
    args.add_argument('--result_path', default='OOD_result', type=str)

    args.add_argument('--bpr_num_neg', default=1, type=int)
    args.add_argument('--split', default=0.4, type=float)
    # LightGCN model
    args.add_argument('--model', default='GSDA', type=str)
    args.add_argument('--decay', default=0.0001, type=float)
    args.add_argument('--lr', default=0.001, type=float)
    args.add_argument('--batch_size', default=2048, type=int)
    args.add_argument('--layers', default=6, type=float)
    args.add_argument('--eps', default=0.2, type=float)
    args.add_argument('--cl_rate', default=5, type=float)
    args.add_argument('--temperature', default=0.2, type=float)
    args.add_argument('--seed', default=12345, type=int)
    args.add_argument('--align_reg', default=400, type=float)
    args.add_argument('--weight', default=2, type=float)
...
def main(config):
    ...
    file_name = '_'.join((str(config.weight),str(config.layers),str(config.cl_rate),str(config.align_reg),timestamp))
...
if __name__ == '__main__':
    ...
    f.write('\n')
    f.write('\n layers:{}===align_reg:{}====weight:{}===cl-rate:{}====split:{}\n  best_hr@20:{}=====best_recall@20:{}====best_ndcg@20:{}\n test_OOD_hr@20:{:.6f}   test_OOD_recall@20:{:.6f}   test_OOD_ndcg@20:{:.6f}\n test_IID_hr@20:{:.6f}   test_IID_recall@20:{:.6f}   test_IID_ndcg@20:{:.6f} \n  Resulst_path:{}\n '
            .format(config.layers,config.align_reg,config.weight,config.cl_rate,config.split,val_hr,val_recall,val_ndcg,test_OOD_hr, test_OOD_recall, test_OOD_ndcg ,test_IID_hr, test_IID_recall, test_IID_ndcg,result_path))
    f.write('\n')                                        
    f.close()
